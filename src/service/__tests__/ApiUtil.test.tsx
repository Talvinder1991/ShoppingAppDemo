import 'react-native';
import { commonApiAxios } from '../ApiUtil';
...
describe('Should render correctly', () => {
  test('the data is homeList', async () => {
    const data = await commonApiAxios(Constants.GET,
      Config.BASE_URL + EndPointUrl.HOME_LIST,
      {});
    expect(data?.data).toStrictEqual(cartData);
  });

  test('the fetch fails with an error', async () => {
    try {
      await commonApiAxios(Constants.GET,
        Config.BASE_URL + '',
        {});;
    } catch (e) {
